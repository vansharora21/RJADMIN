        ...
        >
            ...
            <div className='h-full relative overflow-hidden flex flex-col border-r border-gray-700/50'>
                ...
                <div className="relative z-10 p-4 flex flex-col h-full">
                    ...
                    {/* Navigation */}
                    <nav className='flex-grow space-y-3'> {/* Changed from space-y-2 to space-y-3 */}
                        {SIDEBAR_ITEMS.map((item, index) => (
                            <Link key={item.href} to={item.href}>
                                <motion.div 
                                    className='flex items-center p-3 text-sm font-medium rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 hover:bg-white/15 hover:border-white/20 transition-all duration-300 shadow-lg hover:shadow-xl group'
                                ...
                                >
                                    ...
                                    <div className="relative">
                                        <item.icon 
                                            ...
                                            style={{ 
                                                color: item.color, 
                                                minWidth: "10px", // Changed from "0px" to "20px" for proper spacing
                                    ...
                                    <AnimatePresence>
                                        {isSidebarOpen && (
                                            <motion.span
                                                className='ml-4 whitespace-wrap text-white font-medium drop-shadow-sm'
                                                initial={{ opacity: 0, width: 0 }}
                                                animate={{ opacity: 1, width: "auto" }}
                                                exit={{ opacity: 0, width: 0 }}
                                    ...
                                    {/* Hover indicator */}
                                    <motion.div
                                        className="absolute right-2 w-1 h-8 bg-gradient-to-b from-purple-400 to-pink-400 rounded-full opacity-0 group-hover:opacity-100"
                                        initial={{ scale: 0 }}
                                        whileHover={{ scale: 1 }}
